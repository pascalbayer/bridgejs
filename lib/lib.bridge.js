...
var underscore = typeof global != 'undefined' ? require('./lib.underscore.js') : window._;
 ...
 * @type {Bridge}
 */
(function (Bridge, underscore, database, socketio) {
    var db = database;
    var socket = socketio;
        ...
        var broadcastTo = function (name, data, ids) {
            for (iosocket in socket.sockets) {
                for (id in ids) {
                    ...
                    console.log('Socket: ' + socket.sockets[iosocket].id + ', ID: ' + ids[id]);
                }
            }
        };
    ...
    var initSocketProtocol = function () {
        if (Bridge.isServer) {
            ...
            Bridge.Socket.on('insert', function (data) {
                ...
                for (collection in publishedCollections[data.collection]) {
                    ...
                    // TODO: Check if data.data does match the cursor

                    // TODO: Iterate over subscribers, ignore inserting client, when is inside the subscribers array
                    //Bridge.Socket.broadcastTo('insert', data, publishedCollections[data.collection].subscribers);
                }
                /*var Posts = Bridge.Collection('Posts');
                 Posts.find().sort({_id: 1}).toArray(function (items) {
                 console.log(items);
                 });*/
                //Bridge.Socket.broadcastTo('insert', data, publishedCollections[data.collection].subscribers);
                //Bridge.Socket.broadcast('insert', data);
            });
            ...
            Bridge.Socket.on('subscribe', function (data) {
                ...
                for (collection in publishedCollections) {
                ...
                Bridge.Socket.emit('publish', {Error:'test'});
            });
        }
        else {
            Bridge.ready(function () {
                Bridge.Socket.on('insert', function (data) {
                    ...
                    for (binding in db[data.collection].bindings) {
    ...
    init();
}(Bridge, underscore, database, socketio));
...
/*var Collection = (function () {
 ...
 Collection.insert = function (data) {
 if (Bridge.isServer) {
 if (Collection.checkInsertRules(data.collection)) {
 Bridge.Collection.insert(data.data);
 }
 else {
 Bridge.Socket.emit('error', {
 type:'Insertion Error',
 exception:Localization[Bridge.Settings.language].Error.Collection.notPrivileged
 });
 }
 }
 };

 Collection.checkInsertRules = function () {
